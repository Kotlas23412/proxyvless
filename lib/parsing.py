...
def parse_vless_url(vless_url: str) -> dict | None:
    ...
    try:
        ...
        # Поддержка параметров транспортов для VLESS (ws / h2 / grpc),
        # т.к. `build_xray_config()` использует wsPath/wsHost/grpcServiceName.
        # В CIDR-листах обычно встречаются `type=ws&host=...&path=...`.
        ws_path = get("path", "") or get("wsPath", "")
        ws_host = get("host", "") or get("wsHost", "")
        grpc_service_name = get("serviceName", "") or get("grpcServiceName", "")
...
def load_merged_keys(links_file: str) -> tuple[str, list[tuple[str, str]]]:
    ...
    result: list[tuple[str, str]] = []
    total_urls = len(urls)
    # source_stats: (номер, дата, ключей_всего_в_источнике, ключей_валидных_для_parse_proxy_url)
    source_stats: list[tuple[int, str, int, int]] = []
    ...
    with Progress(
        ...
        for idx, url in enumerate(urls, 1):
            try:
                ...
                # Предварительная фильтрация "битых" строк, которые выглядят как поддерживаемые
                # (по префиксу), но не парсятся в структуру. Иначе они попадают в общий пул,
                # но затем ломают/засоряют проверки на следующих этапах.
                parsed_valid: list[tuple[str, str]] = []
                for link, full in parsed:
                    if parse_proxy_url(link):
                        parsed_valid.append((link, full))
                new_count = 0
                for link, full in parsed_valid:
                    if link not in seen_links:
                        seen_links.add(link)
                        result.append((link, full))
                        new_count += 1
                source_stats.append((idx, date_str or "-", len(parsed), len(parsed_valid)))

                progress.update(
                    task,
                    advance=1,
                    description=(
                        f"[cyan]Парсинг ссылок...[/cyan] [{idx}/{total_urls}] "
                        f"получено {len(parsed)} ключей, валидных {len(parsed_valid)}, "
                        f"новых {new_count}, всего: {len(result)}"
                    )
                )
            except (requests.RequestException, requests.exceptions.InvalidURL, OSError, ValueError) as e:
                error_msg = _short_error(str(e))
                source_stats.append((idx, "-", 0, 0))
    ...
    console.print("| № | Дата обновления | Ключей | Валидных |")
    console.print("|---|-----------------|--------|---------|")
    for num, date, count_all, count_valid in source_stats:
        console.print(f"| {num} | {date} | {count_all} | {count_valid} |")
    console.print("")
